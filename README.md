...
* [`plan-build`](pi-extensions/plan-build) - Manage a persistent tmux-backed builder session `builder` with fixed model/session invariants for planner→builder workflows. Exposes the `plan_build` tool and `/plan-build` / `/pb` explicit lifecycle commands.
