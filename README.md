...
* [`lsp.ts`](pi-extensions/lsp.ts) - Lazy auto-start LSP utility (`lsp_query`) modeled after opencode: adds implementation + call hierarchy operations, concise workspace-focused output by default, plus automatic LSP diagnostics feedback after `write`/`edit` tool calls.
