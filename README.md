...
* [`lsp.ts`](pi-extensions/lsp.ts) - Lazy auto-start LSP utility (`lsp_query`) for agent-internal semantic navigation/analysis: adds implementation + call hierarchy operations, concise workspace-focused output by default, plus automatic LSP diagnostics feedback after `write`/`edit` tool calls.
