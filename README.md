...
* [`detour.ts`](pi-extensions/detour.ts) - Side-question mode (`/detour`, `/detour-end [--summary]`) that branches from current context, enforces read-only tools during detour, and returns to the original branch on exit.
