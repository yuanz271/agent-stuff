...
* [`lsp.ts`](pi-extensions/lsp.ts) - Lazy auto-start background LSP integration for agent coding loops: warms language servers and injects concise diagnostics after `write`/`edit` tool calls, with lightweight debug commands (`/lsp-status`, `/lsp-reload`).
