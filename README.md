...
* [`plan-build`](pi-extensions/plan-build) - Manage a persistent tmux-backed builder session `builder` with fixed model/session invariants for planner→builder workflows. Exposes the `plan_build` tool and `/plan-build` / `/pb` commands; bare command toggles start/stop, while explicit args support `start`, `status`, and `stop`.
