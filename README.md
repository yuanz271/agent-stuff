...
* [`plan-mode`](pi-extensions/plan-mode) - Manage a persistent tmux-backed builder session `B` with fixed model/session invariants for planner→builder workflows. Exposes the `plan_mode` tool and `/plan-mode` / `/pm` commands.
