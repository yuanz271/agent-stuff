...
* Added `plan-build` as a planner/builder mode controller with explicit `start` / `on` / `status` / `off` / `stop`, read-only planner mode, planner-model switching with restore on `off`, `/build` delegation to a persistent tmux-backed builder session, layered settings via bundled/global/project `plan-build-settings.yaml` files, split builder `model`/`thinking` settings (with legacy `model: provider/id:thinking` shorthand still accepted), default planner settings that omit `bash` unless re-enabled in global/project overrides, and a default builder target of `openai-codex/gpt-5.3-codex` with thinking `off`.
